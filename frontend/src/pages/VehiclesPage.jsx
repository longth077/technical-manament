...
const COLUMNS = [
  ...
  { key: 'operatingHours', label: 'Số giờ HĐ' },
  { key: 'kilometers', label: 'Số KM' },
];
...
export default function VehiclesPage() {
  ...
  const load = useCallback(async () => {
    const { data: list, error } = await vehicleService.getAll();
  ...
  const handleDelete = async (row) => {
    ...
    const { error } = await vehicleService.remove(row._id || row.id);
    if (error) setAlert({ message: error, type: 'error' });
  ...
  const handleSubmit = async (formData) => {
    ...
    const { error } = editing
      ? await vehicleService.update(editing._id || editing.id, formData)
  ...
  return (
    <div className="page">
      ...
      {modal && (
        <FormModal
          title={modal.editing ? 'Sửa phương tiện' : 'Thêm phương tiện'}
          fields={FORM_FIELDS}
