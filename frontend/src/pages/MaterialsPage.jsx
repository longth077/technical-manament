...
export default function MaterialsPage() {
  ...
  const load = useCallback(async () => {
    const { data: list, error } = await materialService.getAll();
  ...
  const handleDelete = async (row) => {
    ...
    const { error } = await materialService.remove(row._id || row.id);
    if (error) setAlert({ message: error, type: 'error' });
  ...
  const handleSubmit = async (formData) => {
    ...
    const { error } = editing
      ? await materialService.update(editing._id || editing.id, formData)
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div className="page-header-actions">
          <ExcelButtons
            exportUrl="/api/excel/materials/export"
            importUrl="/api/excel/materials/import"
      ...
      {modal && (
        <FormModal
          title={modal.editing ? 'Sửa vật tư' : 'Thêm vật tư'}
          fields={FORM_FIELDS}
