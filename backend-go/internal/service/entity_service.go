...
func (s *EntityService) Create(entity string, payload map[string]any) (map[string]any, error) {
	...
	clean(payload)
	if err := s.db.Table(entity).Create(&payload).Error; err != nil {
		return nil, err
	}
	if id, ok := payload["id"]; ok {
		var row map[string]any
		s.db.Table(entity).Where("id = ?", id).Take(&row)
		return row, nil
	}
	return payload, nil
}
...
func (s *EntityService) Update(entity string, id string, payload map[string]any) (map[string]any, error) {
	...
	clean(payload)
	res := s.db.Table(entity).Where("id = ?", id).Updates(payload)
	if res.Error != nil {
	...
	var row map[string]any
	if err := s.db.Table(entity).Where("id = ?", id).Take(&row).Error; err != nil {
		return nil, err
	}
	return row, nil
}
...
func (s *EntityService) Delete(entity string, id string) error {
	...
	res := s.db.Table(entity).Where("id = ?", id).Delete(nil)
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
